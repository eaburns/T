...
// To drive asyncronous events, the HandleTick method
...
func (b *Box) HandleResize(size image.Point) {
	b.size = size
	dirtyLines(b)
}
...
// HandleTick handles periodic ticks that drive
...
func (b *Box) HandleMove(pt image.Point) bool {
	b.pt = pt
...
// HandleWheel handles the event of the mouse wheel rolling
// and returns whether the text box image needs to be redrawn.
// 	-y is roll up.
// 	+y is roll down.
// 	-x is roll left.
// 	+x is roll right.
func (b *Box) HandleWheel(x, y int) bool {
	now := b.now()
...
// HandleClick handles a mouse button press or release event
// and returns whether the text box image needs to be redrawn.
//
...
// HandleDir handles a keyboard directional event
// and returns whether the text box image needs to be redrawn.
//
...
func (b *Box) HandleDir(x, y int) bool {
	switch {
	case x == -1:
		at := leftRight(b, "-")
		b.cursorCol = -1
...
// HandleMod handles a modifier key state change event
// and returns whether the text box image needs to be redrawn.
//
...
func (b *Box) HandleMod(m int) bool {
	switch {
...
// HandleRune handles the event of a rune being typed
// and returns whether the text box image needs to be redrawn.
//
...
func (b *Box) HandleRune(r rune) bool {
	switch r {
	case '\b':
		if b.dot.At[0] == b.dot.At[1] {
			ed(b, ".-#1,.d")
